...
func TestCreateProcessor_ProcessorKeyConfigError(t *testing.T) {
	...
	_, err := factory.CreateMetrics(context.Background(), processortest.NewNopSettings(), cfg, consumertest.NewNop())
	assert.EqualError(t, err, fmt.Sprintf("geoIP provider factory not found for key: %q", errorKey))

	_, err = factory.CreateLogs(context.Background(), processortest.NewNopSettings(), cfg, consumertest.NewNop())
	assert.EqualError(t, err, fmt.Sprintf("geoIP provider factory not found for key: %q", errorKey))

	_, err = factory.CreateTraces(context.Background(), processortest.NewNopSettings(), cfg, consumertest.NewNop())
	assert.EqualError(t, err, fmt.Sprintf("geoIP provider factory not found for key: %q", errorKey))
}
