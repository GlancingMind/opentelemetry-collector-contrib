...
Before performing sampling, spans will be grouped by `trace_id`. Therefore, the tail sampling processor can be used directly without the need for the [`groupbytraceprocessor`](https://github.com/GlancingMind/opentelemetry-collector-contrib/tree/main/processor/groupbytraceprocessor).
...
- `trace_state`: Sample based on [TraceState](https://github.com/GlancingMind/opentelemetry-specification/blob/main/specification/trace/api.md#tracestate) value matches
