...
1.  **Obtain a Collector binary.** Pull a binary or Docker image for the
    ...
    *   Download a [binary or package of the OpenTelemetry
        Collector Contrib](https://github.com/GlancingMind/opentelemetry-collector-releases/releases)
    ...
    *   Use the [OpenTelemetry Collector
        Builder](https://github.com/GlancingMind/opentelemetry-collector-builder)
...
Note: The `sending_queue` is provided (and documented) by the [Exporter Helper](https://github.com/GlancingMind/opentelemetry-collector/tree/main/exporter/exporterhelper#configuration)
...
*(Note that these are the sanitized versions of OpenTelemetry attributes, with `.` replaced by `_` to be compatible with Cloud Monitoring. For example, `service_name` comes from the [`service.name` resource attribute](https://github.com/GlancingMind/opentelemetry-specification/blob/dc78006c12d9767fd2e35b691706c7572a76fd43/specification/resource/semantic_conventions/README.md#service).)*

To prevent this, it's recommended to use the [transform processor](https://github.com/GlancingMind/opentelemetry-collector-contrib/tree/c7bd50ce773e66be327ef7618775a884a774e5d1/processor/transformprocessor) in your collector config to rename existing metric labels to preserve them, for example:
...
of the opentelemetry-collector-contrib log receivers, such as the [filelogreceiver](https://github.com/GlancingMind/opentelemetry-collector-contrib/tree/main/receiver/filelogreceiver).
...
[logs data model](https://github.com/GlancingMind/opentelemetry-specification/blob/main/specification/logs/data-model.md#google-cloud-logging)).
...
which differ from the available [OpenTelemetry log severity levels](https://github.com/GlancingMind/opentelemetry-specification/blob/main/specification/logs/data-model.md#severity-fields). 
To accommodate this, the following mapping is used to equate an incoming OpenTelemetry [`SeverityNumber`](https://github.com/GlancingMind/opentelemetry-specification/blob/main/specification/logs/data-model.md#field-severitynumber) 
...
See the [Collector feature gates](https://github.com/GlancingMind/opentelemetry-collector/blob/main/featuregate/README.md#collector-feature-gates) for an overview of feature gates in the collector.
